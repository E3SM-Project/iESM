  ...
  subroutine initialize_short_lived_species(ncid_ini, pbuf2d)
    ...
    do m=1,nslvd
       ...
       do lchnk=begchunk,endchunk
         call pbuf_set_field(pbuf_get_chunk(pbuf2d, lchnk), pbf_idx, tmpptr, start=(/1,1,m/),kount=(/pcols,pver,1/))
       end do
    enddo
...
!---------------------------------------------------------------------
  subroutine set_short_lived_species( q, lchnk, ncol, pbuf )
    ...
    do m=1,nslvd
       n = map(m)
       call pbuf_set_field(pbuf, pbf_idx, q(:ncol,:,n))
    enddo
...
!---------------------------------------------------------------------
  subroutine get_short_lived_species( q, lchnk, ncol, pbuf )
    ...
    do m=1,nslvd
       n = map(m)
       call pbuf_get_field(pbuf, pbf_idx, tmpptr, start=(/1,1,m/), kount=(/ncol,pver,1/))
       q(:,:,n) = tmpptr(:,:)
    enddo
