...
!-------------------------------------------------------------------------------
  subroutine sat_hist_readnl(nlfile, hfilename_spec, mfilt, fincl, nhtfrq, avgflag_pertape)
    ...
    namelist /satellite_options_nl/ sathist_track_infile, sathist_hfilename_spec, sathist_fincl, sathist_mfilt, sathist_nclosest, sathist_ntimestep
...
!-------------------------------------------------------------------------------
  subroutine sat_hist_write( tape , nflds, nfils)
    ...
    call get_indices( obs_lats, obs_lons, ncols, nocols, col_ndxs, chk_ndxs, fdyn_ndxs, ldyn_ndxs, phs_owners, dyn_owners, mlats, mlons, phs_dists, dyn_dists )
...
!-------------------------------------------------------------------------------
  subroutine read_next_position( ncols )
    ...
    if ( first_datetime > enddatetime ) then
       if (masterproc) write(iulog,'(a,2f16.6)') 'sat_hist->read_next_position: all of the satellite date times are after the time window', first_datetime, enddatetime
       return
    endif
    if ( last_datetime < begdatetime ) then
       if (masterproc) write(iulog,'(a,2f16.6)') 'sat_hist->read_next_position: all of the satellite date times are before the time window', begdatetime, last_datetime
       return
    endif
...
!-------------------------------------------------------------------------------
  ...
  subroutine get_indices( lats, lons, ncols, nocols, col_ndxs, chk_ndxs, fdyn_ndxs, ldyn_ndxs, phs_owners, dyn_owners, mlats, mlons, phs_dists, dyn_dists )
    ...
    allocate(ichks(sathist_nclosest),icols(sathist_nclosest),idyn1s(sathist_nclosest),idyn2s(sathist_nclosest),iphs_owners(sathist_nclosest),idyn_owners(sathist_nclosest))
    allocate(rlats(sathist_nclosest), rlons(sathist_nclosest), plats(sathist_nclosest), plons(sathist_nclosest), iphs_dists(sathist_nclosest), idyn_dists(sathist_nclosest))
    ...
    do i = 1,ncols
       ...
       call dyn_grid_find_gcols( lat, lon, sathist_nclosest, idyn_owners(:), idyn1s(:), idyn2s(:), rlats(:), rlons(:), idyn_dists(:) )
