  ...
  !--------------------------------------------------- !
   ...
   real(r8), private            :: icecrit                      ! Critical RH for ice clouds in Wilson & Ballard closure ( smaller = more ice clouds )
 ...
 ! Other
   ...
   QQw_all(:ncol,top_lev:)    = QQw_prog(:ncol,top_lev:)  + QQw1(:ncol,top_lev:) + QQw2(:ncol,top_lev:) + qlten_pwi1(:ncol,top_lev:) + qlten_pwi2(:ncol,top_lev:) + A_ql_adj(:ncol,top_lev:)
   QQi_all(:ncol,top_lev:)    = QQi_prog(:ncol,top_lev:)  + QQi1(:ncol,top_lev:) + QQi2(:ncol,top_lev:) + qiten_pwi1(:ncol,top_lev:) + qiten_pwi2(:ncol,top_lev:) + A_qi_adj(:ncol,top_lev:)
   ...
   QQnl_all(:ncol,top_lev:)   = QQnl_prog(:ncol,top_lev:)  + QQnl1(:ncol,top_lev:) + QQnl2(:ncol,top_lev:) + nlten_pwi1(:ncol,top_lev:) + nlten_pwi2(:ncol,top_lev:) + ACnl(:ncol,top_lev:) + A_nl_adj(:ncol,top_lev:)
   QQni_all(:ncol,top_lev:)   = QQni_prog(:ncol,top_lev:)  + QQni1(:ncol,top_lev:) + QQni2(:ncol,top_lev:) + niten_pwi1(:ncol,top_lev:) + niten_pwi2(:ncol,top_lev:) + ACni(:ncol,top_lev:) + A_ni_adj(:ncol,top_lev:)
   ...
   s_tendout(:ncol,top_lev:)  = cp*( T_star(:ncol,top_lev:)  -  T0(:ncol,top_lev:) )/dt - cp*(A_T(:ncol,top_lev:)+C_T(:ncol,top_lev:))
   qv_tendout(:ncol,top_lev:) =    ( qv_star(:ncol,top_lev:) - qv0(:ncol,top_lev:) )/dt - (A_qv(:ncol,top_lev:)+C_qv(:ncol,top_lev:))
   ql_tendout(:ncol,top_lev:) =    ( ql_star(:ncol,top_lev:) - ql0(:ncol,top_lev:) )/dt - (A_ql(:ncol,top_lev:)+C_ql(:ncol,top_lev:))
   qi_tendout(:ncol,top_lev:) =    ( qi_star(:ncol,top_lev:) - qi0(:ncol,top_lev:) )/dt - (A_qi(:ncol,top_lev:)+C_qi(:ncol,top_lev:))
   nl_tendout(:ncol,top_lev:) =    ( nl_star(:ncol,top_lev:) - nl0(:ncol,top_lev:) )/dt - (A_nl(:ncol,top_lev:)+C_nl(:ncol,top_lev:))
   ni_tendout(:ncol,top_lev:) =    ( ni_star(:ncol,top_lev:) - ni0(:ncol,top_lev:) )/dt - (A_ni(:ncol,top_lev:)+C_ni(:ncol,top_lev:))
...
!
             hlatvp = hlatv - 2369.0_r8*(tlim(i)-tt0)
             hlatsb = hlatv
             if (tlim(i) < tt0) then
               hltalt(i) = hlatsb
             else
...
! No icephs or water to ice transition
!
               hlatvp = hlatv - 2369.0_r8*(tsp(i)-tt0)
               hlatsb = hlatv
               if (tsp(i) < tt0) then
                 hltalt(i) = hlatsb
               else
         ...
         if (error_found) then
            do i = 1,ncol
               if (doit(i) == 0) then
                  ...
                  call endrun ('cldwat2m_macro::findsp_water_onelayer -- not converging')
               endif
            end do
         endif
      endif
      ...
      if (error_found) then
         do i = 1,ncol
            if (doit(i) == 2 .and. abs((enin(i)-enout(i))/(enin(i)+enout(i))) > 1.e-4_r8) then
               ...
               write (iulog,*) ' t, q, p, enin ', t(i), q(i), p(i), enin(i)
               write (iulog,*) ' tsp, qsp, enout ', tsp(i), qsp(i), enout(i)
               call endrun ('cldwat2m_macro::findsp_water_onelayer -- enthalpy is not conserved')
            endif
         end do
      endif
...
! No icephs or water to ice transition
!
               hlatvp = hlatv - 2369.0_r8*(tsp(i,k)-tt0)
               hlatsb = hlatv
               if (tsp(i,k) < tt0) then
                 hltalt(i,k) = hlatsb
               else
         ...
         if (error_found) then
            do i = 1,ncol
               if (doit(i) == 0) then
                  ...
                  call endrun ('cldwat2m_macro::findsp_water -- not converging')
               endif
            end do
         endif
      endif
      ...
      if (error_found) then
         do i = 1,ncol
            if (doit(i) == 2 .and. abs((enin(i)-enout(i))/(enin(i)+enout(i))) > 1.e-4_r8) then
               ...
               write (iulog,*) ' t, q, p, enin ', t(i,k), q(i,k), p(i,k), enin(i)
               write (iulog,*) ' tsp, qsp, enout ', tsp(i,k), qsp(i,k), enout(i)
               call endrun ('cldwat2m_macro::findsp_water -- enthalpy is not conserved')
            endif
         end do
      endif
