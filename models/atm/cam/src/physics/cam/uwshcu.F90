...
25     continue
 ...
 45    continue
       ...
       !                if we use new 'qlten,qiten', there is no problem.
        ...
        call positive_moisture_single( xlv, xls, mkx, dt, qmin(1), qmin(ixcldliq), qmin(ixcldice), dp0, qv0_star, ql0_star, qi0_star, s0_star, qvten, qlten, qiten, sten )
         ...
         if (error_found) then
            do i = 1,ncol
               if (doit(i) == 0) then
                  ...
                  call endrun ('uwshcu::FINDSP -- not converging')
               endif
            end do
         endif
      endif
      ...
      if (error_found) then
         do i = 1,ncol
            if (doit(i) == 2 .and. abs((enin(i)-enout(i))/(enin(i)+enout(i))) > 1.e-4_r8) then
               ...
               write(iulog,*) ' t, q, p, enin ', t(i,k), q(i,k), p(i,k), enin(i)
               write(iulog,*) ' tsp, qsp, enout ', tsp(i,k), qsp(i,k), enout(i)
               call endrun ('uwshcu::FINDSP -- enthalpy is not conserved')
            endif
         end do
      endif
