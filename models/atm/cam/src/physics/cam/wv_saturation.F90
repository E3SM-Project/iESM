...
!  Compute saturation vapor pressure by using one of the following:
!
!    - Goff and Gatch (1946)
!    - Murphy and Koop (2005)
!
!  NOTE: The function to be used is selected by the namelist
!  variable wvsat_scheme in phys_cntl.
...
!  type refers to saturation with respect to liquid (0) or ice (1)
      ...
      character(len=16)  :: wvsat_scheme                   ! Water Vapor Scheme


      ! Check which scheme to use.
      call phys_getopts(wvsat_scheme_out=wvsat_scheme)
      
      if (wvsat_scheme == "GoffGratch") then
      ...
      if (type.eq.0) then
      ...
      else if (wvsat_scheme == "MurphyKoop") then

        ! ice
        if (type.eq.1) then

          ! (good down to 110 K)
          polysvp = exp(9.550426_r8 - (5723.265_r8 / t) + (3.53068_r8 * log(t)) - (0.00728332_r8 * t))
        ...
        if (type.eq.0) then
          ...
          polysvp = exp(54.842763_r8 - (6763.22_r8 / t) - (4.210_r8 * log(t)) + (0.000367_r8 * t) + &
            (tanh(0.0415_r8 * (t - 218.8_r8)) * &
            (53.878_r8 - (1331.22_r8 / t) - (9.44523_r8 * log(t)) + 0.014025_r8 * t)))
        end if
        
      else
        call endrun("wv_saturation::ERROR: Unknown water vapor saturation vapor pressure scheme.")
      end if
...
real(r8) function qsat_water(t,p)
  ...
  qsat_water = epsqs*es/(p-(1._r8-epsqs)*es) ! saturation w/respect to liquid only
  if(qsat_water < 0._r8) qsat_water = 1._r8
...
subroutine vqsat_water(t,p,qsat_water,len)
  ...
  coef = hlatv/rgasv
  do i=1,len
     es = 611._r8*exp(coef*(t0inv-1._r8/t(i)))
     qsat_water(i) = epsqs*es/(p(i)-(1._r8-epsqs)*es) ! saturation w/respect to liquid only
     if(qsat_water(i) < 0._r8) qsat_water(i) = 1._r8
  enddo
...
real(r8) function qsat_ice(t,p)
  ...
  real(r8) es ! saturation vapor pressure (Pa)
  real(r8), parameter :: t0inv = 1._r8/273._r8
  es = 611._r8*exp((hlatv+hlatf)/rgasv*(t0inv-1._r8/t))
  qsat_ice = epsqs*es/(p-(1._r8-epsqs)*es) ! saturation w/respect to liquid only
  if(qsat_ice < 0._r8) qsat_ice = 1._r8
...
subroutine vqsat_ice(t,p,qsat_ice,len)
  ...
  do i=1,len
     es = 611._r8*exp(coef*(t0inv-1._r8/t(i)))
     qsat_ice(i) = epsqs*es/(p(i)-(1._r8-epsqs)*es) ! saturation w/respect to liquid only
     if(qsat_ice(i) < 0._r8) qsat_ice(i) = 1._r8
  enddo
