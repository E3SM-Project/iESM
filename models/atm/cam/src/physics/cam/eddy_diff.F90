  ...
  real(r8)                    :: lbulk_max      =  40.e3_r8     !  Maximum master length scale designed to address issues in the upper atmosphere
                                                                !  where vertical model resolution is coarse [ m ].
                                                                !  In order not to disturb turbulence characteristics in the lower troposphere,
                                                                !  this should be set at least larger than ~ a few km.  
  real(r8)                    :: leng_max(pver) =  40.e3_r8     !  Maximum length scale designed to address issues in the upper atmosphere.
  ...
  subroutine init_eddy_diff( kind, pver, gravx, cpairx, rairx, zvirx, & 
    ...
    real(r8), intent(in) :: eddy_max_bot_pressure  ! Bottom pressure level (hPa) at which namelist leng_max and lbulk_max are applied
