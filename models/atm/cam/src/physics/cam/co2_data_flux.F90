...
!  integer,  parameter :: totsz=2000           ! number greater than data time sample
  ...
  character(len=256)    :: co2_input_file
...
! If model time is past current forward data timeslice reread dynamic dataset to see if more data has
! been added.
! need to clean this up, shouldn't be deallocating space previously allocated by different routine.
 
     if ( caldayloc > xin%cdayfp .and. .not. (xin%np1_f==1 .and. caldayloc > xin%cdayfm) ) then
        if ( xin%np1_f+1 > xin%timesz ) then
           if (masterproc) &
              call handle_ncerr( nf90_close(xin%ncid_f),'co2_data_flux.F90:359')
           deallocate( xin%co2flx)
           deallocate( xin%co2bdy)
           deallocate(xin%date_f)
           deallocate(xin%sec_f)
           deallocate(xin%xvar)
           call read_data_flux(co2_input_file, xin)
        end if
     end if
