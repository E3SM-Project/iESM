...
interface
   ...
   integer, parameter :: r8 = selected_real_kind(12)
   real(r8)             :: polysvp
   real(r8), intent(in) :: T     !  T is input in units of K.
   integer,  intent(in) :: type  !  type refers to saturation with respect to
   !  liquid (0) or ice (1)
...
subroutine micro_mg_init( &
     ...
     rhoh2o, tmelt_in, latvap, latice, epsqs_in, &
     rhmini_in, gamma, errstring)
...
real(r8),         intent(in)  :: epsqs_in        ! Ratio of h2o to dry air molecular weights 
real(r8),         intent(in)  :: rhmini_in       ! Minimum rh for ice cloud fraction > 0.
real(r8),         external    :: gamma           ! gamma function
...
subroutine micro_mg_tend ( &
     sub_column, pcols, pver, ncol, top_lev, deltatin,&
     tn, qn, qc, qi, nc,                              &
...
real(r8), external :: polysvp

character(128),   intent(out) :: errstring       ! Output status (non-blank for error return)

external vqsatd_water
...
do k=top_lev,pver
   ...
   call vqsatd_water(t(:,k), p(:,k), es, qs, gammas, ncol) ! use rhw
      ...
      do k=top_lev,pver
         ...
         if (qric(i,k).ge.qsmall .and. qcic(i,k).ge.qsmall) then
            ...
            if (sub_column) then
               ...
               pra(k) = cons12/(cons3*cons20)*67._r8*(qcic(i,k)*qric(i,k))**1.15_r8
               npra(k) = pra(k)/(qcic(i,k)/ncic(i,k))
