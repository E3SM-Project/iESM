...
module carma_intr
  ...
  use physics_types,  only: physics_state, physics_ptend, physics_ptend_init, set_wet_to_dry, &
                            set_dry_to_wet
...
!    do_wave_emit(nlwbands+1:) = .FALSE.
    ...
    !
    ! NOTE: We only want to do this with RRTMG. If CAM_RT is being used as indicated
    ! by having 19 shortwave bands, then skip this. This is a bad way to detect this,
    ! but is all that is available now.
    if ((masterproc) .and. (nsrest == 0) .and. (nswbands /= 19) .and. (carma_do_optics)) then
...
2   format(i6,4x,3(1pe11.3,4x))
    ...
    call CARMA_InitializeModel(carma, rc)
  ...
  subroutine carma_timestep_tend(state, cam_in, cam_out, ptend, dt, pbuf, dlf, rliq, prec_str, snow_str, &
    ...
    do icol = 1, state%ncol
      ...
      xc(:) = state%lon(icol) / DEG2RAD
      yc(:) = state%lat(icol) /  DEG2RAD
      ...
      if (single_column) then
        ...
        if (abs(state%lat(icol) /  DEG2RAD) >= (90._r8 - (90._r8 / (plat-1)))) then
          if (state%lat(icol) < 0._r8) then
            yc(:) = yc(:) + 0.25_r8 * dlat
          else
            yc(:) = yc(:) - 0.25_r8 * dlat
          endif    
      ...
      if (carma_do_pheat) then
        ...
        if (is_first_step()) then
          ...
          do iwvl = 1, nlwbands
            do iz = 1, pver
              lu_ptr(icol, iz, iwvl) = planckIntensity(wave(iwvl), state%t(icol, iz)) / 1e7_f * 1e4_f * dwave(iwvl) * PI
            end do
      ...
      do ielem = 1, NELEM
        ...
        if (cnsttype == I_CNSTTYPE_PROGNOSTIC) then
          ...
          do ibin = 1, NBIN
            ...
            ! is prognostic and thus are not advected in the paerent model.
            if (ibin <= maxbin) then
              call CARMASTATE_SetBin(cstate, ielem, ibin, state%q(icol, :, icnst4elem(ielem, ibin)), rc)
      ...
      do igas = 1, NGAS
        ...
        if (is_first_step()) then
          gc_ptr(icol,:) = state%q(icol, :, icnst4gas(igas))
          sati_ptr(icol,:) = -1._f
          satl_ptr(icol,:) = -1._f
        end if

        call CARMASTATE_SetGas(cstate, igas, state%q(icol, :, icnst4gas(igas)), rc, &
      ...
      call CARMA_DiagnoseBins(carma, cstate, state, pbuf, icol, dt, rc, rliq=rliq, prec_str=prec_str, snow_str=snow_str)
      ...
      if (do_detrain) then
        call CARMA_Detrain(carma, cstate, cam_in, dlf, state, icol, dt, rc, rliq=rliq, prec_str=prec_str, &
      ...
      call CARMA_CloudFraction(carma, cstate, cam_in, state, icol, cldfrc, rhcrit, rc)
      ...
      if (do_drydep) then
        ...
        if (cam_in%ocnfrac(icol) > 0._r8) then
          call CARMA_calcram(ocnfv, &
                             zzocen, &
                             state%pdel(icol, pver), &
                             state%pmid(icol, pver), &
                             state%t(icol, pver), &
                             obklen(icol), &
                             ocnram)
        end if
      ...
      if (present(rliq)) then
        call CARMA_DiagnoseBulk(carma, cstate, cam_out, state, pbuf, ptend, icol, dt, rc, &
      ...
      dz(:) = state%zi(icol, 1:pver) - state%zi(icol, 2:pverp)
        ...
        do ibin = 1, NBIN
          ...
          if (cnsttype == I_CNSTTYPE_PROGNOSTIC) then
            ...
            if (ibin <= maxbin) then
              ...
              ptend%lq(icnst) = .true.
              ptend%q(icol, :, icnst) = (newstate(:) - state%q(icol, :, icnst)) / dt
      ...
      if (carma_do_thermo) then 
        ptend%ls = .true.
        ptend%s(icol, :) = (newstate(:) - state%t(icol, :)) * cpair / dt
      endif
...
!$OMP END CRITICAL
    ...
    call carma_output_diagnostics(state, ptend, gpdiags, sbdiags, gsdiags, spdiags, bndiags)


    ! Convert any dry constituents back to a dry mass mixing ratio.
      ...
      if ((do_mie) .and. (cnsttype == I_CNSTTYPE_PROGNOSTIC)) then
        ...
        do ibin = 1, NBIN
          ...
          if (ibin <= maxbin) then
            ...
            ! Create the file.
            call wrap_create(filepath, NF_CLOBBER, fid)
            ...
            dimids(1) = rhdim
            call wrap_def_var(fid, 'rh',  NF_DOUBLE, 1, dimids, rhvar)
            
            dimids(1) = lwdim
            call wrap_def_var(fid, 'lw_band', NF_DOUBLE, 1, dimids, lwvar)
            
            dimids(1) = swdim
            call wrap_def_var(fid, 'sw_band', NF_DOUBLE, 1, dimids, swvar)
            ...
            dimids(1) = rhdim
            dimids(2) = lwdim
            call wrap_def_var(fid, 'abs_lw', NF_DOUBLE, 2, dimids, abs_lw_var)
            
            call wrap_put_att_text(fid, abs_lw_var, 'units', 'meter^2 kilogram^-1') 
            
            dimids(1) = rhdim
            dimids(2) = swdim
            call wrap_def_var(fid, 'ext_sw', NF_DOUBLE, 2, dimids, ext_sw_var)
            call wrap_def_var(fid, 'ssa_sw', NF_DOUBLE, 2, dimids, ssa_sw_var)
            call wrap_def_var(fid, 'asm_sw', NF_DOUBLE, 2, dimids, asm_sw_var)
            ...
            dimids(1) = swdim
            call wrap_def_var(fid, 'refindex_real_aer_sw', NF_DOUBLE, 1, dimids, sw_r_refidx_var)
            call wrap_def_var(fid, 'refindex_im_aer_sw',   NF_DOUBLE, 1, dimids, sw_i_refidx_var)
            
            dimids(1) = lwdim
            call wrap_def_var(fid, 'refindex_real_aer_lw', NF_DOUBLE, 1, dimids, lw_r_refidx_var)
            call wrap_def_var(fid, 'refindex_im_aer_lw',   NF_DOUBLE, 1, dimids, lw_i_refidx_var)
            ...
            dimids(1) = omdim
            call wrap_def_var(fid, 'opticsmethod',  NF_CHAR, 1, dimids, omvar)
  
            call wrap_def_dim(fid, 'namelength',  20, andim)
            dimids(1) = andim
            call wrap_def_var(fid, 'aername',  NF_CHAR, 1, dimids, anvar)
  
            call wrap_def_dim(fid, 'name_len',  32, namedim)
            dimids(1) = namedim
            call wrap_def_var(fid, 'name',  NF_CHAR, 1, dimids, namevar)
  
            call wrap_def_var(fid, 'density',            NF_DOUBLE, 0, dimids, denvar)
            call wrap_def_var(fid, 'sigma_logr',         NF_DOUBLE, 0, dimids, slogvar)
            call wrap_def_var(fid, 'dryrad',             NF_DOUBLE, 0, dimids, dryrvar)
            call wrap_def_var(fid, 'radmin_aer',         NF_DOUBLE, 0, dimids, rminvar)
            call wrap_def_var(fid, 'radmax_aer',         NF_DOUBLE, 0, dimids, rmaxvar)
            call wrap_def_var(fid, 'hygroscopicity',     NF_DOUBLE, 0, dimids, hygrovar)
            call wrap_def_var(fid, 'num_to_mass_ratio',  NF_DOUBLE, 0, dimids, ntmvar)
            ...
            call wrap_put_var_realx(fid, sw_i_refidx_var, imag(refidx(nlwbands+1:)))
            call wrap_put_var_realx(fid, lw_r_refidx_var, real(refidx(:nlwbands)))
            call wrap_put_var_realx(fid, lw_i_refidx_var, imag(refidx(:nlwbands)))
            ...
            call wrap_put_var_realx(fid, abs_lw_var, abs_lw(:nrh, :))
            
            ! Write out the shortwave fields.
            call wrap_put_var_realx(fid, ext_sw_var, ext_sw(:nrh, :))
            call wrap_put_var_realx(fid, ssa_sw_var, ssa_sw(:nrh, :))
            call wrap_put_var_realx(fid, asm_sw_var, asm_sw(:nrh, :))
  ...
  subroutine CARMA_CreateRefTFile(carma, filepath, lev, reft, rc)
    ...
    dimids(1) = levdim
    call wrap_def_var(fid, 'lev',  NF_DOUBLE, 1, dimids, levvar)
    ...
    call wrap_def_var(fid, 'T', NF_DOUBLE, 1, dimids, tvar)
