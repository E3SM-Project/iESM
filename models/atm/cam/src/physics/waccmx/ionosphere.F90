...
!---------------------------------------------------------------------------------
  ...
  use physics_buffer, only : pbuf_add_field, pbuf_get_index,dtype_r8,physics_buffer_desc, pbuf_get_field !! Needed to get variables from physics buffer
...
!-----------------------------------------------------------------------
    ...
    call pbuf_add_field('ElecTemp','global',dtype_r8,(/pcols,pver/),idx) ! Electron temperature (global so can write to history files) 

    call pbuf_add_field('IonTemp', 'global',dtype_r8,(/pcols,pver/),idx) ! Ion temperature (global so can write to history files)
