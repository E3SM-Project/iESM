...
!   use shr_orb_mod,      only: shr_orb_params
   ...
   use rad_driver_routines, only: rad_driver_init
...
!-----------------------------------------------------------------------
   ...
   real(r8) :: landm( pcols, begchunk:endchunk )
   ...
   time_loop: do irec = 1,ntimes
      ...
      call get_rad_data_input( phys_state, pbuf2d, cam_in, landm, irec )
      call t_stopf('DRIVER_INPUT_DATA')

      call t_startf('DRIVER_CHUNK_LOOP')
      !$OMP PARALLEL DO PRIVATE (c, fsns, fsnt, flns, flnt, fsds, net_flx, pbuf)
