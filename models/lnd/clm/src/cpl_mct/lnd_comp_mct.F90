...
! IAC coupling
!
  integer, pointer :: iac_EClock(:)
  type(iac_cdata_type) :: iac_cdata
  real*8, pointer :: iac_x2iac(:,:)
  real*8, pointer :: iac_iac2x(:,:)
  logical :: iac_present
  logical :: iac_prognostic
...
! !USES:
    ...
    use clm_time_manager , only : get_curr_date, get_nstep, get_step_size, set_timemgr_init, &
                                  set_nextsw_cday,is_first_restart_step
...
! !LOCAL VARIABLES:
    ...
    integer :: ymd,yr,mon,day,tod                    ! time variables for setting iac_EClock - should come from seq eclock get data calls
...
!-----------------------------------------------------------------------
    ...
    ! Call iac init method
    if(masterproc) then
       allocate(iac_EClock(iac_EClock_size))
       iac_EClock = 0

       call get_curr_date( yr, mon, day, tod )
       ymd = yr*10000 + mon*100 + day
       iac_EClock(iac_EClock_ymd) = ymd
       iac_EClock(iac_EClock_tod) = tod
       iac_EClock(iac_EClock_dt)  = dtime_sync

       allocate(iac_cdata%i(iac_cdata_size))
       allocate(iac_cdata%r(iac_cdata_size))
       allocate(iac_cdata%l(iac_cdata_size))
       allocate(iac_cdata%c(iac_cdata_size))
       iac_cdata%r = 0.0
       iac_cdata%i = 0
       iac_cdata%l = .false.
       iac_cdata%c = ' '

       iac_cdata%i(iac_cdatai_logunit) = iulog
       iac_cdata%c(iac_cdatac_casename) = trim(caseid)
       iac_cdata%l(iac_cdatal_rest) = is_first_restart_step()

       !--- note, no data passed in/out via iac_x2iax or iac_iac2x
       call iac_init_mod(iac_EClock, iac_cdata, iac_x2iac, iac_iac2x)

       iac_present = iac_cdata%l(iac_cdatal_iac_present)
       iac_prognostic = iac_cdata%l(iac_cdatal_iac_prognostic)
       write(iulog,*) trim(sub),' : iac_present   = ',iac_present
       write(iulog,*) trim(sub),' : iac_prognostic = ',iac_prognostic
    endif
...
!---------------------------------------------------------------------------
    ...
    ! Call iac run method
    if (masterproc .and. iac_present) then
       iac_cdata%i(iac_cdatai_logunit) = iulog
       iac_cdata%l(iac_cdatal_rest) = is_first_restart_step()
       iac_EClock(iac_EClock_ymd) = ymd
       iac_EClock(iac_EClock_tod) = tod_sync

       !--- note, no data passed in/out via iac_x2iax or iac_iac2x
       !--- at the present time.  all done with files.
       !--- clm h1 hist files as input to iac, 
       !--- clm pftdyn file is output from iac
       call iac_run_mod(iac_EClock, iac_cdata, iac_x2iac, iac_iac2x)

    endif   ! iac run method
