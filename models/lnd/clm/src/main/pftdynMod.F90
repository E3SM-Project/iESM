...
! ! PRIVATE TYPES
  ...
  real(r8), allocatable   :: wtpft1(:,:)   
  real(r8), allocatable   :: wtpft2(:,:)
...
!   Because we are dynamically adding to the dynamic pft file we need
!   to check if there is a new timesample 

    ! Obtain pft years from dynamic landuse file

    call ncd_pio_openfile (ncid, locfn, 0)
    call ncd_inqdid(ncid, 'time', varid) 
    call ncd_inqdlen(ncid, varid, ntimes_new)

    if (ntimes_new.ne.ntimes) then
       ntimes=ntimes_new
       if (allocated(yearspft)) deallocate (yearspft)
       allocate (yearspft(ntimes), stat=ier)
       if (ier /= 0) then
          write(iulog,*) subname//' allocation error for yearspft'; call endrun()
       end if
