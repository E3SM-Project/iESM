...
!=======================================================================
  ...
  subroutine seq_map_map( mapper, av_s, av_d, fldlist, norm, avwts_s, avwtsfld_s, string )
    ...
    if (mapper%copy_only) then
       ...
       if (present(fldlist)) then
          call mct_aVect_copy(aVin=av_s,aVout=av_d,rList=fldlist,vector=usevector)
       else
          call mct_aVect_copy(aVin=av_s,aVout=av_d,vector=usevector)
       endif
       ...
       !-------------------------------------------
       if (present(fldlist)) then
          call mct_rearr_rearrange_fldlist(av_s, av_d, mapper%rearr, VECTOR=usevector, &
               ALLTOALL=usealltoall, fldlist=fldlist)
       else
          call mct_rearr_rearrange(av_s, av_d, mapper%rearr, VECTOR=usevector, &
